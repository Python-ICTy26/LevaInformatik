...
class Console(UI):
    ...
    def draw_borders(self, screen) -> None:
        ...
        t = (
            screen.getmaxyx()[0] - self.life.cols * self.cell_size - self.top_space - 1
        ) // 2 + self.top_space
    ...
    def draw_grid(self, screen) -> None:
        ...
        t = (
            screen.getmaxyx()[0] - self.life.cols * self.cell_size - self.top_space
        ) // 2 + self.top_space
                ...
                if self.life.curr_generation[i][j] == 1:
                    for k in range(self.cell_size):
                        for p in range(self.cell_size * 2):
                            screen.addch(
                                t + self.cell_size * i + k,
                                l + self.cell_size * j * 2 + p,
                                " ",
                            )
        # Выключаем атрибуты
        screen.attroff(attrs)
